  ...
  return (
    <div className="weight-row">
      ...
      <div className="weight-track">
        <span
          style={{ width: `${clamp(value * 100, 0, 100)}%`, background: color }}
        />
      </div>
    </div>
  );
...
function Settings({
  ...
  return (
    <details className="settings">
      <summary>
        Runtime settings <span>configure the simulation</span>
      </summary>
      <div className="settings-grid">
        ...
        <label>
          ...
          >
            <option value="neural">Neural gate</option>
      ...
      <div className="settings-actions">
        <span className="settings-note">
          Changes affect the backend simulation after Apply.
        </span>
        {error && (
...
export default function Home() {
  ...
  useEffect(() => {
    // Backend config is an external source; state changes happen after the request resolves.
  ...
  }, [reconnectToken, syncConfig]);

  const updateConfig = (key: keyof AppConfig, value: string | number) =>
    setConfig((current) => ({ ...current, [key]: value }) as AppConfig);
  const applyConfig = async () => {
    ...
    try {
      ...
      const body = (await response
        .json()
        .catch(() => null)) as Partial<AppConfig> | null;
  ...
  const statusMessage = useMemo(() => {
    ...
    if (status === "connected" && !hasSnapshot)
      return "Connected; waiting for first snapshot…";
    ...
    <main className="shell">
      <header className="topbar">
        ...
        <div className="topbar-actions">
          ...
          <button
            ...
            onClick={() => setReconnectToken((token) => token + 1)}
          >
      ...
      <section className="intro">
        ...
        <div className="intro-note">
          ...
          <strong>1 Hz gate</strong>
          <span>→ weights only →</span>
          <strong>event-speed quote</strong>
        </div>
      </section>

      <div className={`stream-banner ${status}`} role="status">
        <span className="stream-icon">
          {status === "connected" ? "↗" : "·"}
        </span>
        <span>{statusMessage}</span>
        <span className="stream-endpoint">{API_URL}</span>
      </div>
      ...
      <section className="dashboard-section">
        ...
        >
          <span>
            Normalized snapshots from the backend. Values remain intentionally
            blank until a real snapshot arrives.
          </span>
        </SectionTitle>
        <div className="metric-grid market-metrics">
          <Metric
            label="Mid price"
            value={empty ? "—" : price(state.market.mid)}
            detail={state.symbol}
            tone="accent"
          />
          <Metric
            label="Spread"
            value={empty ? "—" : `${number(state.market.spread_bps, 2)} bps`}
            detail="top of book"
          />
          <Metric
            label="Imbalance"
            value={empty ? "—" : signed(state.market.imbalance)}
            detail="bid pressure"
            tone={state.market.imbalance >= 0 ? "positive" : "negative"}
          />
          <Metric
            label="Trade flow"
            value={empty ? "—" : signed(state.market.trade_flow)}
      ...
      <section className="dashboard-section">
        <SectionTitle
          index="02"
          eyebrow="slow plane"
          title={`${modeTitle(state.gate.mode)} sets the mix`}
        >
          <span>
            Refreshes at its configured cadence. It never emits an order.
          </span>
        </SectionTitle>
        <div className="split-grid">
          <article className="panel gate-panel">
            <div className="panel-heading">
              ...
              <span className="model-version">
                {empty ? "—" : state.gate.model_version}
              </span>
            </div>
            <div className="regime">
              {empty ? "Awaiting feed" : modeTitle(state.gate.mode)}
            </div>
            <div className="confidence">
              <span>Largest weight</span>
              <b>{empty ? "—" : percent(largestWeight(state))}</b>
              <div className="confidence-track">
                <span
                  style={{
                    width: `${empty ? 0 : largestWeight(state) * 100}%`,
                  }}
                />
              </div>
            </div>
            <div className="gate-footer">
              <span>
                {empty ? "Next refresh —" : `Next refresh ${age(nextRefresh)}`}
              </span>
              <span>
                {empty
                  ? "—"
                  : `${state.gate.mode} · ${state.gate.cadence_ms} ms`}
              </span>
            </div>
          </article>
          <article className="panel weights-panel">
            ...
            <WeightRow
              ...
              value={empty ? 0 : state.gate.weights.microprice}
              color="#0b8f7c"
            />
            <WeightRow
              label="Trade-flow impulse"
              value={empty ? 0 : state.gate.weights.flow}
              color="#7568b3"
            />
            <WeightRow
              label="Short reversion"
              value={empty ? 0 : state.gate.weights.reversion}
              color="#b56a3c"
            />
      ...
      <section className="dashboard-section">
        ...
        >
          <span>
            Every contribution stays legible. Scores are event-speed signals,
              ...
              <div className="expert-values">
                <div>
                  ...
                  <b className={expert.score >= 0 ? "up" : "down"}>
                    {empty ? "—" : signed(expert.score)}
                  </b>
                </div>
                <div>
                  <span>weight</span>
                  <b>{empty ? "—" : percent(expert.weight)}</b>
                </div>
                <div>
                  <span>contribution</span>
                  <b className={expert.contribution >= 0 ? "up" : "down"}>
                    {empty ? "—" : signed(expert.contribution)}
                  </b>
                </div>
              </div>
            </article>
        ...
        <div className="panel chart-panel">
          <div className="panel-heading">
            ...
            <span className="chart-window">
              last {history.length || 0} snapshots
            </span>
          </div>
      ...
      <section className="dashboard-section">
        ...
        >
          <span>
            A toy quote and paper ledger make the effect inspectable without
            connecting to an order endpoint.
          </span>
        </SectionTitle>
        <div className="lower-grid">
          ...
          <article className="panel paper-panel">
            ...
            <div className="paper-values">
              <Metric
                label="Inventory"
                value={empty ? "—" : signed(state.paper.inventory, 4)}
                detail="BTC · paper only"
              />
              <Metric
                label="P&amp;L"
                value={empty ? "—" : signed(state.paper.pnl)}
          ...
          <article className="panel health-panel">
            ...
            <div className="health-status">
              <span
                className={`health-dot ${empty ? "waiting" : state.health.feed_status === "running" ? "ok" : "warn"}`}
              />
              <strong>{empty ? "Waiting" : state.health.feed_status}</strong>
            </div>
            <dl>
              <div>
                <dt>Risk reason</dt>
                <dd>
                  {empty ? "—" : state.health.risk_reason || "none reported"}
                </dd>
              </div>
              <div>
                <dt>Message age</dt>
                <dd>{empty ? "—" : age(state.health.message_age_ms)}</dd>
              </div>
              <div>
                <dt>Reconnects</dt>
                <dd>{empty ? "—" : state.health.reconnects}</dd>
              </div>
              <div>
                <dt>Last snapshot</dt>
                <dd>{formatTimestamp(state.timestamp)}</dd>
              </div>
            </dl>
          </article>
        </div>
