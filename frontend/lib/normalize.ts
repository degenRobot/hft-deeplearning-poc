...
export function normalizeState(input: unknown): MarketGateState | null {
  ...
  const rawMarket = (raw.market && typeof raw.market === "object" ? raw.market : {}) as Record<string, unknown>;
  const rawGate = (raw.gate && typeof raw.gate === "object" ? raw.gate : {}) as Record<string, unknown>;
  const rawWeights = (rawGate.weights && typeof rawGate.weights === "object" ? rawGate.weights : {}) as Record<string, unknown>;
  const rawPaper = (raw.paper && typeof raw.paper === "object" ? raw.paper : {}) as Record<string, unknown>;
  const rawHealth = (raw.health && typeof raw.health === "object" ? raw.health : {}) as Record<string, unknown>;
  const rawExperts = Array.isArray(raw.experts) ? raw.experts : [];

  const experts = EMPTY_STATE.experts.map((placeholder, index) => {
    const candidate = (rawExperts[index] && typeof rawExperts[index] === "object" ? rawExperts[index] : {}) as Record<string, unknown>;
    return {
  ...
  return {
    ...
    quote:
      raw.quote && typeof raw.quote === "object"
        ? { bid: finite((raw.quote as Record<string, unknown>).bid), ask: finite((raw.quote as Record<string, unknown>).ask) }
        : null,
